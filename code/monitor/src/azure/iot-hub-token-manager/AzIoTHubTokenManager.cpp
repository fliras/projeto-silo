...
static char SAS_SIGNATURE_BUFFER[256];
...
{
  this->client = client;
  this->deviceKey = AZ_SPAN_FROM_STR(CFG_IOTHUB_DEVICE_KEY);
  this->signatureBuffer = AZ_SPAN_FROM_BUFFER(SAS_SIGNATURE_BUFFER);
  this->sasTokenBuffer = AZ_SPAN_FROM_BUFFER(MQTT_PASSWORD);
